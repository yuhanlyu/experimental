...
bool IsPrime(uint32_t n, const bool prime[]) {
  return (n & 1) == 0 ? (n == 2) : (prime[NumberToIndex(n)] != 0);
}
...
void SieveBit(uint64_t n, uint32_t prime[]) {
  ...
  uint64_t max_index = NumberToBitIndex(n);
...
void ImprovedSieveBit(uint64_t n, uint32_t prime[]) {
  ...
  for (uint64_t i = 3, index = 0; i <= bound; i += 2, ++index) {
    if (BitGet(index, prime)) {
      ...
      uint64_t j_index = NumberToBitIndex(i * k);
      for (int64_t k_index = NumberToBitIndex(k);
           k_index >= static_cast<int64_t>(index); j_index -= i, --k_index) {
