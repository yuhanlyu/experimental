...
static void BM_Sieve(benchmark::State& state) {
  while (state.KeepRunning()) {
    Sieve(state.range(0), prime);
  }
}
BENCHMARK(BM_Sieve)->RangeMultiplier(m)->Range(1024, size);
...
static void BM_SieveBitset(benchmark::State& state) {
  while (state.KeepRunning()) {
    int64_t n = state.range(0);
    SieveBit(n << 10, prime_bit);
  }
}
BENCHMARK(BM_SieveBitset)->RangeMultiplier(m)->Range(1, bitset_size_limit);
