...
static void BM_LinearSieve(benchmark::State& state) {
  while (state.KeepRunning()) {
    LinearSieve(state.range(0), prime);
  }
}
BENCHMARK(BM_LinearSieve)->RangeMultiplier(m)->Range(1024, size);
...
static void BM_LinearSieveBitset(benchmark::State& state) {
  while (state.KeepRunning()) {
    int64_t n = state.range(0);
    LinearSieveBit(n << 10, prime_bit);
  }
}
BENCHMARK(BM_LinearSieveBitset)
    ->RangeMultiplier(m)
